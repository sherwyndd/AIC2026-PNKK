#!/usr/bin/env python3
"""Stage 4: ASR speech extraction with faster-whisper and 6-second keyframe window alignment."""
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(description="Stage 4: ASR Extraction & Keyframe Alignment")
    ...
    parser.add_argument("--wait", action="store_true", default=True, help="Wait for Stage 2 keyframes metadata if not ready")
    parser.add_argument("--no-wait", action="store_false", dest="wait", help="Skip video if Stage 2 keyframes metadata missing")
...
def main():
    ...
    logger.info("Starting Stage 4 ASR Whisper...")
    ...
    logger.info(f"Stage 4 ASR finished. Processed {result['processed_videos']} video(s). Output in {result['output_dir']}")
